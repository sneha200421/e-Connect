    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        b1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                NetworkInfo activenetwork = manager.getActiveNetworkInfo();
                if(null!=activenetwork){
                    FirebaseDatabase.getInstance().getReference().child("videoposts").child(t4.getText().toString()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@androidx.annotation.NonNull Task<Void> task) {
                            FirebaseDatabase.getInstance().getReference().child(FirebaseAuth.getInstance().getUid()+"videoposts").child(t4.getText().toString()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@androidx.annotation.NonNull Task<Void> task) {
                                    Toast.makeText(DisplayVideo2.this, "Post Deleted!", Toast.LENGTH_SHORT).show();
                                    Intent i = new Intent(DisplayVideo2.this,videoimageplayer.class);
                                    startActivity(i);
                                }
                            });
                        }
