...
public class CreateVideoPost extends AppCompatActivity {
    ...
    private void uploadvideo() {
        if (videouri != null) {
            String mycurrenttime= DateFormat.getDateTimeInstance().format(Calendar.getInstance().getTime());
            final StorageReference reference = storageReference.child(mycurrenttime);
    ...
    private void torealtime(){
        ...
        FirebaseDatabase.getInstance().getReference().child("videoposts").child(mycurrenttime2).setValue(u).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
        ...
        FirebaseDatabase.getInstance().getReference().child(FirebaseAuth.getInstance().getUid()+"videoposts").child(mycurrenttime2).setValue(u).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
